...
from skynet.server.address_manager_sqlite_store import create_address_manager_from_db
...
class FullNodeDiscovery:
    ...
    def __init__(
        ...
        self.server: SkynetServer = server
    ...
    async def on_connect(self, peer: ws.WSSkynetConnection):
        if (
    ...
    async def update_peer_timestamp_on_message(self, peer: ws.WSSkynetConnection):
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self):
        ...
        async def on_connect(peer: ws.WSSkynetConnection):
    ...
    async def on_connect_callback(self, peer: ws.WSSkynetConnection):
