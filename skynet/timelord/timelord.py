...
from skynetvdf import create_discriminant, prove
...
from skynet.protocols import timelord_protocol
...
from skynet.timelord.iters_from_block import iters_from_block
from skynet.timelord.timelord_state import LastState
from skynet.timelord.types import Chain, IterationType, StateType
...
class Timelord:
    def __init__(self, root_path, config: Dict, constants: ConsensusConstants):
        ...
        self.server: Optional[SkynetServer] = None
    ...
    async def _start(self):
        ...
        if not self.bluebox_mode:
            ...
            if os.name == "nt" or slow_bluebox:
                # `vdf_client` doesn't build on windows, use `prove()` from skynetvdf.
    ...
    def _can_infuse_unfinished_block(self, block: timelord_protocol.NewUnfinishedBlockTimelord) -> Optional[uint64]:
        ...
        timelord_reward_puzzle_hash: bytes32 = decode_puzzle_hash(self.config["xnt_target_address"])
