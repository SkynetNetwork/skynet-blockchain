...
from skynet.util.bech32m import decode_puzzle_hash, encode_puzzle_hash
from skynet.util.byte_types import hexstr_to_bytes
from skynet.util.ints import uint32, uint64
from skynet.util.keychain import KeyringIsLocked, bytes_to_mnemonic, generate_mnemonic
from skynet.util.path import path_from_root
from skynet.util.ws_message import WsRpcMessage, create_payload_dict
from skynet.wallet.cc_wallet.cc_wallet import CCWallet
from skynet.wallet.derive_keys import master_sk_to_singleton_owner_sk
...
from skynet.wallet.util.backup_utils import download_backup, get_backup_info, upload_backup
from skynet.wallet.util.trade_utils import trade_record_to_dict
from skynet.wallet.util.transaction_type import TransactionType
from skynet.wallet.util.wallet_types import WalletType
from skynet.wallet.wallet_info import WalletInfo
from skynet.wallet.wallet_node import WalletNode
...
class WalletRpcApi:
    def __init__(self, wallet_node: WalletNode):
        ...
        self.service_name = "skynet_wallet"
    ...
    async def _state_changed(self, *args) -> List[WsRpcMessage]:
        ...
        return [create_payload_dict("state_changed", data, "skynet_wallet", "wallet_ui")]
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("xnt_target_address")
        pool_target = config["pool"].get("xnt_target_address")
        found_farmer = False
        found_pool = False
    ...
    async def create_new_wallet(self, request: Dict):
        ...
        if request["wallet_type"] == "cc_wallet":
            ...
            if request["mode"] == "new":
                ...
                from skynet.pools.pool_wallet_info import initial_pool_state_from_dict
