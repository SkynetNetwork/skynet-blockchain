...
from skynet.cmds.init_funcs import check_keys, skynet_init
from skynet.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from skynet.daemon.keychain_server import KeychainServer, keychain_commands
from skynet.daemon.windows_signal import kill
from skynet.plotters.plotters import get_available_plotters
from skynet.plotting.util import add_plot_directory
from skynet.server.server import ssl_context_for_root, ssl_context_for_server
from skynet.ssl.create_ssl import get_mozilla_ca_crt
from skynet.util.skynet_logging import initialize_logging
from skynet.util.config import load_config
from skynet.util.json_util import dict_to_json_str
...
from skynet.util.service_groups import validate_service
from skynet.util.setproctitle import setproctitle
from skynet.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Skynet.")
    quit()
...
service_plotter = "skynet_plotter"
...
if getattr(sys, "frozen", False):
    name_map = {
        "skynet": "skynet",
        "skynet_wallet": "start_wallet",
        "skynet_full_node": "start_full_node",
        "skynet_harvester": "start_harvester",
        "skynet_farmer": "start_farmer",
        "skynet_introducer": "start_introducer",
        "skynet_timelord": "start_timelord",
        "skynet_timelord_launcher": "timelord_launcher",
        "skynet_full_node_simulator": "start_simulator",
        "skynet_seeder": "skynet_seeder",
        "skynet_seeder_crawler": "skynet_seeder_crawler",
        "skynet_seeder_dns": "skynet_seeder_dns",
    }
...
class WebSocketServer:
    ...
    def _skynetpos_plotting_command_args(self, request: Any, ignoreCount: bool) -> List[str]:
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        plotter: str = request.get("plotter", "skynetpos")
        command_args: List[str] = ["skynet", "plotters", plotter]

        command_args.extend(self._common_plotting_command_args(request, ignoreCount))

        if plotter == "skynetpos":
            command_args.extend(self._skynetpos_plotting_command_args(request, ignoreCount))
        elif plotter == "madmax":
    ...
    async def start_plotting(self, request: Dict[str, Any]):
        ...
        plotter = request.get("plotter", "skynetpos")
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("skynet daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered SKYNET_ROOT
    os.environ["SKYNET_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with SKYNET_ROOT: {os.environ['SKYNET_ROOT']}")
    ...
    if service_command == "skynet_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in skynet_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    skynet_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("skynet_daemon")
