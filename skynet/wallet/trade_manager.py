...
from skynet.wallet.cc_wallet import cc_utils
from skynet.wallet.cc_wallet.cc_utils import CC_MOD, SpendableCC, spend_bundle_for_spendable_ccs, uncurry_cc
from skynet.wallet.cc_wallet.cc_wallet import CCWallet
from skynet.wallet.puzzles.genesis_by_coin_id_with_0 import genesis_coin_id_for_genesis_coin_checker
from skynet.wallet.trade_record import TradeRecord
from skynet.wallet.trading.trade_status import TradeStatus
from skynet.wallet.trading.trade_store import TradeStore
from skynet.wallet.transaction_record import TransactionRecord
from skynet.wallet.util.trade_utils import (
            ...
            for id in offer.keys():
                ...
                if isinstance(wallet, CCWallet):
                    ...
                    if spend_bundle is None:
                        ...
                        to_exclude = spend_bundle.removals()
                    new_spend_bundle = await wallet.create_spend_bundle_relative_skynet(amount, to_exclude)
                else:
    ...
    async def respond_to_offer(self, file_path: Path) -> Tuple[bool, Optional[TradeRecord], Optional[str]]:
        ...
        for coinsol in offer_spend_bundle.coin_spends:
            ...
            if r:
                ...
                # standard skynet coin
                unspent = await self.wallet_state_manager.get_spendable_coins_for_wallet(1)
                if coinsol.coin in [record.coin for record in unspent]:
                    return False, None, "can't respond to own offer"
                if skynet_discrepancy is None:
                    skynet_discrepancy = get_output_discrepancy_for_puzzle_and_solution(coinsol.coin, puzzle, solution)
                else:
                    skynet_discrepancy += get_output_discrepancy_for_puzzle_and_solution(coinsol.coin, puzzle, solution)
                coinsols.append(coinsol)

        skynet_spend_bundle: Optional[SpendBundle] = None
        if skynet_discrepancy is not None:
            skynet_spend_bundle = await self.wallet_state_manager.main_wallet.create_spend_bundle_relative_skynet(
                skynet_discrepancy, []
            )
            if skynet_spend_bundle is not None:
                for coinsol in coinsols:
                    skynet_spend_bundle.coin_spends.append(coinsol)
        ...
        for colour in cc_discrepancies.keys():
            if cc_discrepancies[colour] < 0:
                ...
                if skynet_spend_bundle is None:
                    to_exclude: List = []
                else:
                    to_exclude = skynet_spend_bundle.removals()
                ...
                if my_cc_spends is None or my_cc_spends == set():
                    ...
                    if zero_spend_bundle is None:
                        return (
                            False,
                            None,
                            "Unable to generate zero value coin. Confirm that you have skynet available",
                        )
        ...
        now = uint64(int(time.time()))
        if skynet_spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, skynet_spend_bundle])
            if skynet_discrepancy < 0:
                # TODO: address hint error and remove ignore
                ...
                tx_record = TransactionRecord(
                    ...
                    amount=uint64(abs(skynet_discrepancy)),
                    fee_amount=uint64(0),
                    confirmed=False,
                    sent=uint32(10),
                    spend_bundle=skynet_spend_bundle,
                    additions=skynet_spend_bundle.additions(),
                    removals=skynet_spend_bundle.removals(),
                    wallet_id=uint32(1),
                    sent_to=[],
                    trade_id=std_hash(spend_bundle.name() + bytes(now)),
                    type=uint32(TransactionType.OUTGOING_TRADE.value),
                    name=skynet_spend_bundle.name(),
                )
            else:
                # TODO: address hint error and remove ignore
                #       error: Argument "to_puzzle_hash" to "TransactionRecord" has incompatible type "bytes"; expected
