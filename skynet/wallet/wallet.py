...
from skynet.consensus.cost_calculator import calculate_cost_of_program, NPCResult
...
from skynet.types.blockchain_format.program import Program, SerializedProgram
...
from skynet.wallet.puzzles.puzzle_utils import (
...
from skynet.wallet.secret_key_store import SecretKeyStore
from skynet.wallet.sign_coin_spends import sign_coin_spends
...
class Wallet:
    ...
    async def create_spend_bundle_relative_skynet(self, skynet_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None
        ...
        if skynet_amount < 0:
            utxos = await self.select_coins(abs(skynet_amount), exclude)
        else:
        ...
        skynet_amount = spend_value + skynet_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": skynet_amount}]
