...
from skynet.daemon.keychain_proxy import (
    KeychainProxy,
...
from skynet.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from skynet.protocols import wallet_protocol
from skynet.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
...
from skynet.server.node_discovery import WalletPeers
...
from skynet.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED, load_config
from skynet.util.errors import Err, ValidationError
from skynet.util.ints import uint32, uint128
from skynet.util.keychain import Keychain
from skynet.util.lru_cache import LRUCache
from skynet.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from skynet.util.network import get_host_addr
from skynet.util.path import mkdir, path_from_root
from skynet.wallet.block_record import HeaderBlockRecord
from skynet.wallet.derivation_record import DerivationRecord
from skynet.wallet.settings.settings_objects import BackupInitialized
from skynet.wallet.transaction_record import TransactionRecord
from skynet.wallet.util.backup_utils import open_backup_file
from skynet.wallet.util.wallet_types import WalletType
from skynet.wallet.wallet_action import WalletAction
from skynet.wallet.wallet_blockchain import ReceiveBlockResult
from skynet.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'skynet keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSSkynetConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSSkynetConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSSkynetConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSSkynetConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSSkynetConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSSkynetConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
...
async def wallet_next_block_check(
    peer: WSSkynetConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
