...
from skynet.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from skynet.plotters.skynetpos import get_skynetpos_install_info, plot_skynet
from skynet.plotters.madmax import get_madmax_install_info, plot_madmax
from skynet.plotters.install_plotter import install_plotter
...
def build_parser(subparsers, root_path, option_list, name, plotter_desc):
    ...
    for option in option_list:
        ...
        u_default = 0 if name == "skynetpos" else 256
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in SKYNET_ROOT.
    skynet_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within SKYNET_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
    ...
    build_parser(subparsers, root_path, skynet_plotter, "skynetpos", "Skynetpos Plotter")
    ...
    if args.plotter == "skynetpos":
        plot_skynet(args, skynet_root_path)
    if args.plotter == "madmax":
        plot_madmax(args, skynet_root_path, root_path)
    if args.plotter == "bladebit":
        plot_bladebit(args, skynet_root_path, root_path)
...
def get_available_plotters(root_path) -> Dict[str, Any]:
    ...
    skynetpos: Optional[Dict[str, Any]] = get_skynetpos_install_info()
    ...
    if skynetpos is not None:
        plotters["skynetpos"] = skynetpos
