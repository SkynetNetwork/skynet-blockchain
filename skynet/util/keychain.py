...
DEFAULT_USER = f"user-skynet-{CURRENT_KEY_VERSION}"  # e.g. user-skynet-1.8
DEFAULT_SERVICE = f"skynet-{DEFAULT_USER}"  # e.g. skynet-user-skynet-1.8
...
def supports_keyring_passphrase() -> bool:
    # Support can be disabled by setting SKYNET_PASSPHRASE_SUPPORT to 0/false
    return os.environ.get("SKYNET_PASSPHRASE_SUPPORT", "true").lower() in ["1", "true"]
...
def obtain_current_passphrase(prompt: str = DEFAULT_PASSPHRASE_PROMPT, use_passphrase_cache: bool = False) -> str:
    ...
    before failing.
    """
    from skynet.cmds.passphrase_funcs import prompt_for_passphrase
