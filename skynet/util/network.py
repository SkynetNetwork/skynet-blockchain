...
def class_for_type(type: NodeType) -> Any:
    if type is NodeType.FULL_NODE:
        ...
        from skynet.introducer.introducer_api import IntroducerAPI

        return IntroducerAPI
    elif type is NodeType.TIMELORD:
        from skynet.timelord.timelord_api import TimelordAPI

        return TimelordAPI
    elif type is NodeType.FARMER:
        from skynet.farmer.farmer_api import FarmerAPI

        return FarmerAPI
    elif type is NodeType.HARVESTER:
        from skynet.harvester.harvester_api import HarvesterAPI
