...
from skynet.consensus.network_type import NetworkType
from skynet.consensus.pot_iterations import calculate_iterations_quality, calculate_sp_interval_iters
from skynet.farmer.farmer import Farmer
from skynet.protocols import farmer_protocol, harvester_protocol
from skynet.protocols.harvester_protocol import PoolDifficulty
...
class FarmerAPI:
    ...
    async def new_proof_of_space(
        self, new_proof_of_space: harvester_protocol.NewProofOfSpace, peer: ws.WSSkynetConnection
    ):
        """
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.post(
                            ...
                            headers={"User-Agent": f"Skynet Blockchain v.{__version__}"},
                        ) as resp:
                            if resp.ok:
    ...
    @api_request
    @peer_required
    async def respond_plots(self, _: harvester_protocol.RespondPlots, peer: ws.WSSkynetConnection):
