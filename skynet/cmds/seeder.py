...
import skynet.cmds.configure as skynet_configure
from skynet import __version__
from skynet.cmds.skynet import monkey_patch_click
from skynet.cmds.init_funcs import init
from skynet.seeder.util.config import patch_default_seeder_config
from skynet.seeder.util.service_groups import all_groups, services_for_groups
from skynet.seeder.util.service import launch_service, kill_service
...
@click.group(
    help=f"\n  Manage the Skynet Seeder ({__version__})\n",
    epilog="Try 'skynet seeder start crawler' or 'skynet seeder start server'",
...
@cli.command("version", short_help="Show the Skynet Seeder version")
...
def init_cmd(ctx: click.Context, **kwargs):
    print("Calling Skynet Seeder Init...")
    init(None, ctx.obj["root_path"], True)
    if os.environ.get("SKYNET_ROOT", None) is not None:
        print(f"warning, your SKYNET_ROOT is set to {os.environ['SKYNET_ROOT']}.")
    root_path = ctx.obj["root_path"]
    print(f"Skynet directory {root_path}")
    if root_path.is_dir() and not Path(root_path / "config" / "config.yaml").exists():
        # This is reached if SKYNET_ROOT is set, but there is no config
        # This really shouldn't happen, but if we dont have the base skynet config, we can't continue
...
def configure(
    ...
    skynet_configure.configure(root_path, "", "", "", "", "", "", "", "", testnet, "")

    config: Dict = load_config(DEFAULT_ROOT_PATH, "config.yaml")
    change_made = False
    if testnet is not None:
        if testnet == "true" or testnet == "t":
            print("Updating Skynet Seeder to testnet settings")
            port = 59999
            network = "testnet7"
            bootstrap = ["testnet-node.skynet-network.org"]
            ...
            print("Updating Skynet Seeder to mainnet settings")
            port = 9999
            network = "mainnet"
            bootstrap = ["node.skynet-network.org"]
    ...
    if change_made:
        print("Restart any running Skynet Seeder services for changes to take effect")
