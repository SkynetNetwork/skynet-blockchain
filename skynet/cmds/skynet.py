...
from skynet.cmds.configure import configure_cmd
from skynet.cmds.farm import farm_cmd
from skynet.cmds.init import init_cmd
from skynet.cmds.keys import keys_cmd
from skynet.cmds.netspace import netspace_cmd
from skynet.cmds.passphrase import passphrase_cmd
from skynet.cmds.plots import plots_cmd
from skynet.cmds.show import show_cmd
from skynet.cmds.start import start_cmd
...
@click.group(
    help=f"\n  Manage skynet blockchain infrastructure ({__version__})\n",
    epilog="Try 'skynet start node', 'skynet netspace -d 192', or 'skynet show -s'",
...
if not supports_keyring_passphrase():
    from skynet.cmds.passphrase_funcs import remove_passphrase_options_from_cmd

    # TODO: Remove once keyring passphrase management is rolled out to all platforms
    remove_passphrase_options_from_cmd(cli)


@cli.command("version", short_help="Show skynet version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", short_help="Runs skynet daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by skynet start <service>
)
@click.pass_context
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    import asyncio
    from skynet.daemon.server import async_run_daemon
