...
from skynet.consensus.coinbase import create_puzzlehash_for_pk
from skynet.ssl.create_ssl import (
    ensure_ssl_dirs,
    generate_ca_signed_cert,
    get_skynet_ca_crt_key,
    make_ca_cert,
    write_ssl_cert_and_key,
)
from skynet.util.bech32m import encode_puzzle_hash
from skynet.util.config import (
    create_default_skynet_config,
    initial_config_file,
    load_config,
    save_config,
...
from skynet.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'skynet keys generate'")
        return None

    config: Dict = load_config(new_root, "config.yaml")
    pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
    all_targets = []
    stop_searching_for_farmer = "xnt_target_address" not in config["farmer"]
    stop_searching_for_pool = "xnt_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("xnt_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("xnt_target_address"):
    ...
    if "xnt_target_address" not in config["farmer"]:
        print(
            f"Setting the xnt destination for the farmer reward (1/8 plus fees, solo and pooling) to {all_targets[0]}"
        )
        ...
        print(
            ...
            f"{config['farmer']['xnt_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "xnt_target_address" not in config["pool"]:
        print(f"Setting the xnt destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
    ...
    if updated_target:
        print(
            f"To change the XNT destination addresses, edit the `xnt_target_address` entries in"
...
def create_all_ssl(root_path: Path):
    ...
    skynet_ca_crt, skynet_ca_key = get_skynet_ca_crt_key()
    skynet_ca_crt_path = ca_dir / "skynet_ca.crt"
    skynet_ca_key_path = ca_dir / "skynet_ca.key"
    write_ssl_cert_and_key(skynet_ca_crt_path, skynet_ca_crt, skynet_ca_key_path, skynet_ca_key)
    ...
    skynet_ca_crt, skynet_ca_key = get_skynet_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, skynet_ca_crt, skynet_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if skynet_init(root_path, fix_ssl_permissions=fix_ssl_permissions) == 0 and root_path.exists():
                return init(create_certs, root_path, fix_ssl_permissions)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return skynet_init(root_path, fix_ssl_permissions=fix_ssl_permissions)


def skynet_version_number() -> Tuple[str, str, str, str]:
...
def skynet_minor_release_number():
    res = int(skynet_version_number()[2])
    print(f"Install release number: {res}")
    return res


def skynet_full_version_str() -> str:
    major, minor, patch, dev = skynet_version_number()
    return f"{major}.{minor}.{patch}{dev}"


def skynet_init(root_path: Path, *, should_check_keys: bool = True, fix_ssl_permissions: bool = False):
    """
    ...
    """
    if os.environ.get("SKYNET_ROOT", None) is not None:
        print(
            f"warning, your SKYNET_ROOT is set to {os.environ['SKYNET_ROOT']}. "
            f"Please unset the environment variable and run skynet init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Skynet directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if SKYNET_ROOT is set, or if user has run skynet init twice
        # before a new update.
    ...
    if should_check_keys:
        check_keys(root_path)
    print("")
    print("To see your keys, run 'skynet keys show --show-mnemonic-seed'")
