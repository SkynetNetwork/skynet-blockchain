...
from skynet.server.outbound_message import NodeType
from skynet.server.server import SkynetServer, ssl_context_for_client
from skynet.server.ws_connection import WSSkynetConnection
from skynet.ssl.create_ssl import generate_ca_signed_cert
...
async def disconnect_all_and_reconnect(server: SkynetServer, reconnect_to: SkynetServer) -> bool:
...
async def add_dummy_connection(
    server: SkynetServer, dummy_port: int, type: NodeType = NodeType.FULL_NODE
    ...
    generate_ca_signed_cert(
        server.skynet_ca_crt_path.read_bytes(), server.skynet_ca_key_path.read_bytes(), dummy_crt_path, dummy_key_path
    )
    ssl_context = ssl_context_for_client(
        server.skynet_ca_crt_path, server.skynet_ca_key_path, dummy_crt_path, dummy_key_path
    )
...
async def connect_and_get_peer(server_1: SkynetServer, server_2: SkynetServer) -> WSSkynetConnection:
    """
