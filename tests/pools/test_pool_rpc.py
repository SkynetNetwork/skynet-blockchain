...
from skynet.protocols.full_node_protocol import RespondBlock
from skynet.rpc.rpc_server import start_rpc_server
from skynet.rpc.wallet_rpc_api import WalletRpcApi
from skynet.rpc.wallet_rpc_client import WalletRpcClient
from skynet.simulator.simulator_protocol import FarmNewBlockProtocol, ReorgProtocol
...
class TestPoolWalletRpc:
    ...
    async def test_absorb_self(self, one_wallet_node_and_rpc, fee):
        ...
        tr: TransactionRecord = await client.send_transaction(
            1, 100, encode_puzzle_hash(status.p2_singleton_puzzle_hash, "txnt")
        )
        await time_out_assert(
            10,
            full_node_api.full_node.mempool_manager.get_spendbundle,
    ...
    async def test_absorb_pooling(self, one_wallet_node_and_rpc, fee):
        ...
        # Balance stars at 6 XNT and 5 more blocks are farmed, total 22 XNT
    ...
    async def test_leave_pool(self, setup, fee, target_puzzle_hash):
        ...
        try:
            ...
            async def have_skynet():
                await self.farm_blocks(full_node_api, our_ph, 1)
                return (await wallets[0].get_confirmed_balance()) > 0

            await time_out_assert(timeout=WAIT_SECS, function=have_skynet)
