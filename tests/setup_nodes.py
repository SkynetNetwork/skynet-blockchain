...
from skynet.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from skynet.full_node.full_node_api import FullNodeAPI
from skynet.server.start_farmer import service_kwargs_for_farmer
from skynet.server.start_full_node import service_kwargs_for_full_node
from skynet.server.start_harvester import service_kwargs_for_harvester
from skynet.server.start_introducer import service_kwargs_for_introducer
from skynet.server.start_service import Service
from skynet.server.start_timelord import service_kwargs_for_timelord
from skynet.server.start_wallet import service_kwargs_for_wallet
from skynet.simulator.start_simulator import service_kwargs_for_full_node_simulator
from skynet.timelord.timelord_launcher import kill_processes, spawn_process
...
from skynet.util.keychain import bytes_to_mnemonic
...
async def setup_farmer(
    ...
    config["xnt_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "xnt")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["xnt_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "xnt")
