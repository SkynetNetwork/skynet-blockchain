...
from skynetpos import DiskPlotter

from skynet.consensus.coinbase import create_puzzlehash_for_pk
from skynet.plotting.util import stream_plot_info_ph, stream_plot_info_pk
from skynet.protocols import farmer_protocol
from skynet.rpc.farmer_rpc_api import FarmerRpcApi
from skynet.rpc.farmer_rpc_client import FarmerRpcClient
from skynet.rpc.harvester_rpc_api import HarvesterRpcApi
from skynet.rpc.harvester_rpc_client import HarvesterRpcClient
...
from skynet.wallet.derive_keys import master_sk_to_wallet_sk, master_sk_to_pooling_authentication_sk
...
class TestRpc:
    ...
    async def test1(self, simulation):
        ...
        try:
            ...
            await client.set_reward_targets(encode_puzzle_hash(new_ph, "xnt"), encode_puzzle_hash(new_ph_2, "xnt"))
            ...
            await client.set_reward_targets(None, encode_puzzle_hash(new_ph_3, "xnt"))
            ...
            assert config["farmer"]["xnt_target_address"] == encode_puzzle_hash(new_ph, "xnt")
            assert config["pool"]["xnt_target_address"] == encode_puzzle_hash(new_ph_3, "xnt")

            new_ph_3_encoded = encode_puzzle_hash(new_ph_3, "xnt")
